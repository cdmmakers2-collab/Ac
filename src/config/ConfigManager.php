...
class ConfigManager extends ConfigPaths {
	...
	public static function checkConfig() : void {
		...
		if (!is_resource($pluginConfigResource)) {
			$log->critical(self::getData(self::PREFIX) . TextFormat::RED . " Unable to read bundled config.yml!");
		...
		$pluginConfigVersion = $pluginConfig["zuri"]["version"] ?? null;
		if ($config->getNested("zuri.version") === $pluginConfigVersion) {
			return;
		}
